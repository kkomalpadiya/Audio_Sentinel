...
$totalBytes = 11086114085L
$archiveRoot = Join-Path $ProjectRoot "data\raw\_archives"
$partialPath = Join-Path $archiveRoot "musan.tar.gz.partial"
...
if ($null -eq (Get-ChildItem -LiteralPath $target -Recurse -File -ErrorAction SilentlyContinue |
    Where-Object { $_.Name -ne ".gitkeep" } | Select-Object -First 1)) {
    tar -xzf $archivePath -C $rawRoot
}

Write-Output "MUSAN is ready at: $target"
